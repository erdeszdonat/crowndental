...
export async function POST(req: Request) {
  try {
    ...
    // (Amíg nincs OpenAI kulcs, egy intelligens, dinamikus algoritmus generál valósághű eredményt,
    // amely garantálja a "Wow" hatást és bemutatja a saját labor előnyeit.)
    ...
    const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY; // FIGYELEM: Service Role Key kell ide!
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      if (dbError) console.error("Supabase mentési hiba:", dbError);
    }
    ...
    if (resendKey) {
      ...
      await resend.emails.send({
        from: 'Crown Dental <info@crowndental.hu>', // Változtasd meg a saját hitelesített domainedre!
        to: email,
