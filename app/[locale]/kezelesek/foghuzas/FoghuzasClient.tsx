...
import {
  ...
  ArrowRight, Plus, Minus, Activity, Layers, CheckCircle2, Zap
} from 'lucide-react';
...
function ServiceHero() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const query = `*[_type == "treatment" && slug.current == "foghuzas"][0]{"url": coalesce(mainImage.asset->url, heroImage.asset->url)}`;
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-gray-50">
      <div className="absolute top-0 inset-x-0 h-full bg-gradient-to-b from-orange-50/50 to-gray-50" />
      <div className="absolute top-20 right-0 w-[500px] h-[500px] bg-orange-100/50 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3" />
      <div className="relative z-10 container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white shadow-sm border border-gray-100 rounded-full text-orange-600 text-sm font-bold tracking-wide uppercase mb-6">
              <Heart className="w-4 h-4" /> {t('heroBadge')}
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold text-gray-900 mb-6 leading-tight tracking-tight">
              {t('heroTitle').split('\n').map((line, i) => i === 0 ? <span key={i}>{line}<br/></span> : <span key={i} className="text-orange-600">{line}</span>)}
            </h1>
            <p className="text-xl md:text-2xl text-gray-500 leading-relaxed font-light mb-8 max-w-xl">{t('heroDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-orange-600 text-white font-bold rounded-full hover:bg-orange-700 transition-all shadow-lg hover:shadow-orange-600/30">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-gray-100">
              {imageUrl && <img src={imageUrl} alt="Foghúzás" className="w-full h-full object-cover animate-in fade-in duration-700" />}
            ...
            <div className="absolute -bottom-6 -left-6 md:-left-10 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center gap-4">
              <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-600"><Activity className="w-6 h-6" /></div>
              <div>
      ...
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            ...
            <p className="text-lg md:text-xl text-orange-100 mb-10 max-w-2xl mx-auto font-light">{t('ctaDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center gap-3 px-10 py-5 bg-white text-orange-700 font-extrabold rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all group">
...
export default function FoghuzasClient() {
  return (
    <div className="bg-white min-h-screen selection:bg-orange-200 selection:text-orange-900">
      <main>
        <ServiceHero />
        <ProblemSolution />
        <ServicesSection />
