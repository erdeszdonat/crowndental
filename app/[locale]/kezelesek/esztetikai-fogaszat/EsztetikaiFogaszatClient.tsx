...
import {
  Calendar, ChevronDown, Sparkles, Shield, Clock, Banknote, Heart,
  ArrowRight, Plus, Minus, Star, Gem, Palette, Zap, Award, Smile
} from 'lucide-react';
...
function ServiceHero() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const query = `*[_type == "treatment" && slug.current == "esztetikai-fogaszat"][0]{"url": coalesce(mainImage.asset->url, heroImage.asset->url)}`;
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-gray-50">
      <div className="absolute top-0 inset-x-0 h-full bg-gradient-to-b from-rose-50/50 to-gray-50" />
      ...
      <div className="relative z-10 container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white shadow-sm border border-gray-100 rounded-full text-rose-600 text-sm font-bold tracking-wide uppercase mb-6">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold text-gray-900 mb-6 leading-tight tracking-tight">
              {t('heroTitle').split('Crown Dental')[0]}<span className="text-rose-600">Crown Dental</span>{t('heroTitle').split('Crown Dental')[1]}
            </h1>
            <p className="text-xl md:text-2xl text-gray-500 leading-relaxed font-light mb-8 max-w-xl">{t('heroDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-rose-600 text-white font-bold rounded-full hover:bg-rose-700 transition-all shadow-lg hover:shadow-rose-600/30">
              <Calendar className="w-5 h-5" /> {ts('consultationBtn')}
            </Link>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
            <div className="absolute -inset-4 bg-rose-100 rounded-[3rem] -z-10 transform rotate-3"></div>
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-gray-100">
              {imageUrl && <img src={imageUrl} alt="Esztétikai fogászat" className="w-full h-full object-cover animate-in fade-in duration-700" />}
            ...
            <div className="absolute -bottom-6 -left-6 md:-left-10 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center gap-4">
              <div className="w-12 h-12 bg-rose-100 rounded-full flex items-center justify-center text-rose-600"><Star className="w-6 h-6" /></div>
              <div>
    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="p-8 md:p-10 bg-rose-50 rounded-3xl border border-rose-100 shadow-[0_8px_30px_rgb(244,63,94,0.1)]">
            <div className="w-14 h-14 bg-rose-600 text-white rounded-2xl flex items-center justify-center mb-6 shadow-md"><Plus className="w-8 h-8" /></div>
    ...
    <section className="py-24 bg-gray-50 border-t border-gray-100">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-rose-600 font-bold uppercase tracking-widest mb-3">{t('servicesBadge')}</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {services.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="w-16 h-16 bg-rose-50 text-rose-600 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
              <h4 className="text-xl font-bold text-gray-900 mb-3">{s.title}</h4>
              <p className="text-gray-500 leading-relaxed mb-6 flex-1">{s.desc}</p>
              <div className="text-rose-600 font-bold text-lg">{s.price}</div>
            </motion.div>
          ))}
        </div>
      </div>
...
function Benefits() {
  const t = useTranslations('treatmentPages.esztetikai-fogaszat');
  const ts = useTranslations('treatmentPages.shared');
  const icons = [<Shield className="w-6 h-6" />, <Clock className="w-6 h-6" />, <Banknote className="w-6 h-6" />, <Heart className="w-6 h-6" />, <Sparkles className="w-6 h-6" />, <Award className="w-6 h-6" />];
  const benefitItems = t.raw('benefitItems') as Array<{ title: string; desc: string }>;
  return (
    <section className="py-24 bg-white border-t border-gray-100">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-rose-600 font-bold uppercase tracking-widest mb-3">{ts('whyCrownBadge')}</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {benefitItems.map((b, i) => (
            <div key={i} className="p-8 bg-gray-50 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="w-14 h-14 bg-rose-50 text-rose-600 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
  ...
  return (
    <section className="py-24 bg-gray-50">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-16">
          <h2 className="text-rose-600 font-bold uppercase tracking-widest mb-3">{ts('processBadge')}</h2>
        ...
        <div className="space-y-8">
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex-shrink-0 w-16 h-16 bg-rose-50 border-2 border-rose-100 text-rose-600 font-black text-2xl rounded-2xl flex items-center justify-center">{i + 1}.</div>
              ...
              <div className="inline-flex items-center gap-2 bg-rose-50 px-4 py-2 rounded-full border border-rose-100 text-rose-700 font-medium">
      ...
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            className="bg-gradient-to-br from-rose-600 to-pink-600 rounded-[2.5rem] p-10 md:p-16 text-center shadow-2xl shadow-rose-600/20 border border-white/10 relative overflow-hidden">
            ...
            <p className="text-lg md:text-xl text-rose-100 mb-10 max-w-2xl mx-auto font-light">{t('ctaDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center gap-3 px-10 py-5 bg-white text-rose-700 font-extrabold rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all group">
  ...
  return (
    <section className="py-24 bg-gray-50 border-t border-gray-100">
      <div className="container mx-auto px-4 max-w-3xl">
        <div className="text-center mb-16"><h2 className="text-rose-600 font-bold text-4xl mb-4">{ts('faqTitle')}</h2></div>
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
              <button className="w-full px-6 py-5 text-left flex items-center justify-between" onClick={() => setOpenIndex(openIndex === index ? null : index)}>
                ...
                <ChevronDown className={`w-5 h-5 text-rose-600 transition-transform ${openIndex === index ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                {openIndex === index && (
...
export default function EsztetikaiFogaszatClient() {
  return (
    <div className="bg-white min-h-screen selection:bg-rose-200 selection:text-rose-900">
      <main>
        <ServiceHero />
        <ProblemSolution />
        <ServicesSection />
