...
import {
  Calendar, ChevronDown, Sparkles, Shield, Clock, Banknote,
  Heart, ArrowRight, Plus, Minus, Smile
} from 'lucide-react';
...
function ServiceHero() {
  ...
  const t = useTranslations('treatmentPages.fogszabalyozas');
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const query = `*[_type == "treatment" && slug.current == "fogszabalyozas"][0]{"url": coalesce(mainImage.asset->url, heroImage.asset->url)}`;
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold text-gray-900 mb-6 leading-tight tracking-tight">
              {t('heroTitle').split('Crown Dental')[0]}<span className="text-violet-600">Crown Dental</span>{t('heroTitle').split('Crown Dental')[1]}
            </h1>
            <p className="text-xl md:text-2xl text-gray-500 leading-relaxed font-light mb-8 max-w-xl">{t('heroDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-violet-600 text-white font-bold rounded-full hover:bg-violet-700 transition-all shadow-lg hover:shadow-violet-600/30">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-gray-100">
              {imageUrl && <img src={imageUrl} alt="Fogszabályozás" className="w-full h-full object-cover animate-in fade-in duration-700" />}
            ...
            <div className="absolute -bottom-6 -left-6 md:-left-10 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center gap-4">
              <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-green-600"><Smile className="w-6 h-6" /></div>
              <div>
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="p-8 md:p-10 bg-violet-50 rounded-3xl border border-violet-100 shadow-[0_8px_30px_rgb(139,92,246,0.1)]">
            <div className="w-14 h-14 bg-violet-600 text-white rounded-2xl flex items-center justify-center mb-6 shadow-md"><Plus className="w-8 h-8" /></div>
...
function Benefits() {
  const t = useTranslations('treatmentPages.fogszabalyozas');
  const ts = useTranslations('treatmentPages.shared');
  const icons = [<Shield className="w-6 h-6" />, <Smile className="w-6 h-6" />, <Banknote className="w-6 h-6" />, <Clock className="w-6 h-6" />, <Heart className="w-6 h-6" />, <Sparkles className="w-6 h-6" />];
  const benefitItems = t.raw('benefitItems') as Array<{ title: string; desc: string }>;
  return (
    <section className="py-24 bg-gray-50 border-t border-gray-100">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-violet-600 font-bold uppercase tracking-widest mb-3">{ts('whyCrownBadge')}</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {benefitItems.map((b, i) => (
            <div key={i} className="p-8 bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="w-14 h-14 bg-violet-50 text-violet-600 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
  ...
  return (
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-16">
          <h2 className="text-violet-600 font-bold uppercase tracking-widest mb-3">{ts('processBadge')}</h2>
        ...
        <div className="space-y-8">
          {steps.map((s, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-6 bg-gray-50 p-6 md:p-8 rounded-3xl border border-gray-100 items-start md:items-center">
              <div className="flex-shrink-0 w-16 h-16 bg-white border-2 border-violet-100 text-violet-600 font-black text-2xl rounded-2xl flex items-center justify-center shadow-sm">{i + 1}.</div>
      ...
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            ...
            <p className="text-lg md:text-xl text-violet-100 mb-10 max-w-2xl mx-auto font-light">{t('ctaDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center gap-3 px-10 py-5 bg-white text-violet-700 font-extrabold rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all group">
  ...
  return (
    <section className="py-24 bg-white border-t border-gray-100">
      <div className="container mx-auto px-4 max-w-3xl">
        <div className="text-center mb-16"><h2 className="text-violet-600 font-bold text-4xl mb-4">{ts('faqTitle')}</h2></div>
...
export default function FogszabalyozasClient() {
  return (
    <div className="bg-white min-h-screen selection:bg-violet-200 selection:text-violet-900">
      <main>
        <ServiceHero />
        <ProblemSolution />
        <Benefits />
