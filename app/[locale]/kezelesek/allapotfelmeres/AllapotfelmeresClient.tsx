...
import {
  Calendar, ChevronDown, Shield, Clock, Banknote, Heart, ArrowRight,
  Plus, Minus, Search, FileCheck, Stethoscope, ClipboardCheck, Eye, Activity
} from 'lucide-react';
...
function ServiceHero() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const query = `*[_type == "treatment" && slug.current == "allapotfelmeres"][0]{"url": coalesce(mainImage.asset->url, heroImage.asset->url)}`;
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white shadow-sm border border-gray-100 rounded-full text-teal-600 text-sm font-bold tracking-wide uppercase mb-6">
              <Search className="w-4 h-4" /> {t('heroBadge')}
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold text-gray-900 mb-6 leading-tight tracking-tight">
              {t('heroTitle').split('Crown Dental')[0]}<span className="text-teal-600">Crown Dental</span>{t('heroTitle').split('Crown Dental')[1]}
            </h1>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-gray-100">
              {imageUrl && (
                <img src={imageUrl} alt="Fogászati állapotfelmérés" className="w-full h-full object-cover animate-in fade-in duration-700" />
              )}
            ...
            <div className="absolute -bottom-6 -left-6 md:-left-10 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center gap-4">
              <div className="w-12 h-12 bg-teal-100 rounded-full flex items-center justify-center text-teal-600"><FileCheck className="w-6 h-6" /></div>
              <div>
                <div className="text-gray-500 text-sm font-medium">{t('heroPriceLabel')}</div>
                <div className="text-2xl font-extrabold text-teal-600">{t('heroPriceValue')}</div>
              </div>
            </div>
          </motion.div>
        </div>
    ...
    <section className="py-24 bg-gray-50 border-t border-gray-100">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-teal-600 font-bold uppercase tracking-widest mb-3">{t('includedBadge')}</h2>
        ...
        <div className="space-y-8">
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex-shrink-0 w-16 h-16 bg-teal-50 border-2 border-teal-100 text-teal-600 font-black text-2xl rounded-2xl flex items-center justify-center">{i + 1}.</div>
              ...
              <div className="inline-flex items-center gap-2 bg-teal-50 px-4 py-2 rounded-full border border-teal-100 text-teal-700 font-medium">
...
function AppointmentCTASection() {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-teal-200/40 rounded-full blur-[100px]" />
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            className="bg-gradient-to-br from-teal-600 to-cyan-600 rounded-[2.5rem] p-10 md:p-16 text-center shadow-2xl shadow-teal-600/20 border border-white/10 relative overflow-hidden">
            ...
            <p className="text-lg md:text-xl text-teal-100 mb-10 max-w-2xl mx-auto font-light">{t('ctaDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center gap-3 px-10 py-5 bg-white text-teal-700 font-extrabold rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all group">
...
export default function AllapotfelmeresClient() {
  return (
