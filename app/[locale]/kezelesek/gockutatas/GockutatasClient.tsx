...
import {
  ...
  ArrowRight, Plus, Minus, Activity, Search, Microscope, CheckCircle2
} from 'lucide-react';
...
function ServiceHero() {
  ...
  const t = useTranslations('treatmentPages.gockutatas');
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const query = `*[_type == "treatment" && slug.current == "gockutatas"][0]{"url": coalesce(mainImage.asset->url, heroImage.asset->url)}`;
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-gray-50">
      <div className="absolute top-0 inset-x-0 h-full bg-gradient-to-b from-slate-100/50 to-gray-50" />
      <div className="absolute top-20 right-0 w-[500px] h-[500px] bg-slate-200/50 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3" />
      <div className="relative z-10 container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white shadow-sm border border-gray-100 rounded-full text-slate-600 text-sm font-bold tracking-wide uppercase mb-6">
            ...
            <Link href={`${p}/idopont`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-slate-700 text-white font-bold rounded-full hover:bg-slate-800 transition-all shadow-lg hover:shadow-slate-600/30">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-gray-100">
              {imageUrl && <img src={imageUrl} alt="Fogászati Góckutatás" className="w-full h-full object-cover animate-in fade-in duration-700" />}
            ...
            <div className="absolute -bottom-6 -left-6 md:-left-10 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-600"><Search className="w-6 h-6" /></div>
              <div>
    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="p-8 md:p-10 bg-slate-50 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(71,85,105,0.1)]">
            <div className="w-14 h-14 bg-slate-700 text-white rounded-2xl flex items-center justify-center mb-6 shadow-md"><Plus className="w-8 h-8" /></div>
...
function ServicesSection() {
  const t = useTranslations('treatmentPages.gockutatas');
  const icons = [<Activity className="w-8 h-8" />, <Search className="w-8 h-8" />, <Microscope className="w-8 h-8" />];
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {services.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="w-16 h-16 bg-slate-100 text-slate-700 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
              ...
              <div className="text-slate-700 font-extrabold text-xl">{s.price}</div>
            </motion.div>
          ))}
        </div>
      </div>
...
function Benefits() {
  const t = useTranslations('treatmentPages.gockutatas');
  const ts = useTranslations('treatmentPages.shared');
  const icons = [<CheckCircle2 className="w-6 h-6" />, <Shield className="w-6 h-6" />, <Sparkles className="w-6 h-6" />, <Heart className="w-6 h-6" />];
  const benefitItems = t.raw('benefitItems') as Array<{ title: string; desc: string }>;
  return (
    <section className="py-24 bg-white border-t border-gray-100">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-slate-600 font-bold uppercase tracking-widest mb-3">{ts('whyCrownBadge')}</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {benefitItems.map((b, i) => (
            <div key={i} className="p-8 bg-gray-50 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="w-14 h-14 bg-slate-200 text-slate-700 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
  ...
  return (
    <section className="py-24 bg-gray-50">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-16">
          <h2 className="text-slate-600 font-bold uppercase tracking-widest mb-3">{ts('processBadge')}</h2>
        ...
        <div className="space-y-8">
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex-shrink-0 w-16 h-16 bg-slate-100 border-2 border-slate-200 text-slate-700 font-black text-2xl rounded-2xl flex items-center justify-center shadow-sm">{i + 1}.</div>
              ...
              <div className="inline-flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-full border border-slate-200 text-slate-700 font-medium shadow-sm">
...
function AppointmentCTASection() {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-slate-200/40 rounded-full blur-[100px]" />
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            className="bg-gradient-to-br from-slate-700 to-slate-900 rounded-[2.5rem] p-10 md:p-16 text-center shadow-2xl shadow-slate-600/20 border border-white/10 relative overflow-hidden">
            ...
            <p className="text-lg md:text-xl text-slate-200 mb-10 max-w-2xl mx-auto font-light">{t('ctaDesc')}</p>
            <Link href={`${p}/idopont`} className="inline-flex items-center gap-3 px-10 py-5 bg-white text-slate-800 font-extrabold rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all group">
  ...
  return (
    <section className="py-24 bg-white border-t border-gray-100">
      <div className="container mx-auto px-4 max-w-3xl">
        <div className="text-center mb-16"><h2 className="text-slate-600 font-bold text-4xl mb-4">{ts('faqTitle')}</h2></div>
...
export default function GockutatasClient() {
  return (
