...
import { ChevronDown, Sparkles, Shield, Clock, Heart, ArrowRight, Plus, Minus, Activity, Search, Microscope, CheckCircle2 } from 'lucide-react';


function ServiceHero({ imageUrl }: { imageUrl: string }) {
  return <TreatmentHero imageUrl={imageUrl} contentKey="gockutatas" />;
}
    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="p-8 md:p-10 bg-sky-50 rounded-3xl border border-sky-200 shadow-[0_8px_30px_rgb(71,85,105,0.1)]">
            <div className="w-14 h-14 bg-sky-700 text-white rounded-2xl flex items-center justify-center mb-6 shadow-md"><Plus className="w-8 h-8" /></div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {services.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="w-16 h-16 bg-sky-100 text-sky-700 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
              ...
              <div className="text-sky-700 font-extrabold text-xl">{s.price}</div>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {benefitItems.map((b, i) => (
            <div key={i} className="p-8 bg-gray-50 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="w-14 h-14 bg-sky-200 text-sky-700 rounded-2xl flex items-center justify-center mb-6">{icons[i]}</div>
        ...
        <div className="space-y-8">
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex-shrink-0 w-16 h-16 bg-sky-100 border-2 border-sky-200 text-sky-700 font-black text-2xl rounded-2xl flex items-center justify-center shadow-sm">{i + 1}.</div>
              ...
              <div className="inline-flex items-center gap-2 bg-sky-50 px-4 py-2 rounded-full border border-sky-200 text-sky-700 font-medium shadow-sm">
      ...
      <div className="relative z-10 container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            className="bg-gradient-to-br from-sky-700 to-sky-900 rounded-[2.5rem] p-10 md:p-16 text-center shadow-2xl shadow-sky-600/20 border border-white/10 relative overflow-hidden">
            ...
            <p className="text-lg md:text-xl text-sky-200 mb-10 max-w-2xl mx-auto font-normal">{t('ctaDesc')}</p>
...
export default function GockutatasClient({ imageUrl }: { imageUrl: string }) {
  return (
